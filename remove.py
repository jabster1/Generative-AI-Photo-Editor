...
@torch.no_grad()
def panoptic_segments(pil_img, processor, model, device, score_thresh=0.7, mask_thresh=0.5):
    W, H = pil_img.size
...
def select_segments(segs, targets, positions, W, H):
    ...
    def pos_ok(center):
        ...
        if "center" in positions and abs(cx-W*0.5) < W*0.3: ok=True
...
def process_image(
    ...
    score_thresh=0.7, mask_thresh=0.5, steps=40, strength=0.80,
